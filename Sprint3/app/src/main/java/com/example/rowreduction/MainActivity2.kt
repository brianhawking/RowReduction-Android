...
class MainActivity2 : AppCompatActivity() {
    ...
    var pivotElement = arrayOf(0,0)
    var showingPivot = false
...
//    lateinit var textViewResult: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pivotButton.setOnClickListener {
            ...
            println("SHOWING PIVOT: $showingPivot")
            pivotElement = matrix.findPivot(numberOfEquations, numberOfVariables,0,0)
            println("pivot row is ${pivotElement[0]}")
            if(pivotElement[0] != -1) {
                val element: TextView = findViewById(equationIDs[pivotElement[0]][pivotElement[1]])

                println("do i show or no?")
                if (showingPivot) {
                    // hide pivot
                        println("hide the pivot now")
                    element.setBackgroundColor(Color.TRANSPARENT)
                    showingPivot = false
                    pivotButton.text = getString(R.string.showPivot)
                }
                else {
                    println("show the pivot now")
                    element.setBackgroundResource(R.drawable.my_border)
                    showingPivot = true
                    pivotButton.text = getString(R.string.hidePivot)
                }
            }
            else {
                println("no elibile pivot")
            }
